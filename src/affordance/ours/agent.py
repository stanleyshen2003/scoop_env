...
def vild_detection(rgb_img_path, object_name, params) -> bool:
    """Use ViLD to detect the object

    Args:
        rgb_img_path (str): path to image
        object_name (List[str]): objects that need to be detected
    """
    ...
    saved_model_dir = '/home/hcis-s17/multimodal_manipulation/scoop_env/src/affordance/ckpt/image_path_v2' 
...
class Affordance_agent_ours(Affordance_agent):
    ...
    def tool_on_hand(self, rgb_img_path, tool_name) -> bool:
        """Use ViLD to detect the tool on hand

        Args:
            rgb_img_path (str): path to current observation image
            tool_name (List[str]): list of tool names
        """
        pass