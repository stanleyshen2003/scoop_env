...
class IsaacSim():
    def __init__(self, userDefinedSettings=None):
        ...
        self.ball_radius, self.ball_mass, self.ball_friction, self.ball_layer_num, self.goal_powder_amount, self.gravity = self.domainInfo.get_domain_parameters()
        ...
        self.gym.subscribe_viewer_keyboard_event(self.viewer, gymapi.KEY_F, "test_pos")
        self.gym.subscribe_viewer_keyboard_event(self.viewer, gymapi.KEY_G, "test_neg")
        self.gym.subscribe_viewer_keyboard_event(self.viewer, gymapi.KEY_P, "scoop")
        ...
        while n > 0:
            ...
            for j in range(n):
                ...
                for k in range(n):
                    ...
                    ball_handle = self.gym.create_actor(env_ptr, self.ball_asset, ball_pose, "grain", collision_group, collision_filter)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(num_envs):
            ...
            self.gym.set_actor_scale(env_ptr, self.bowl, 0.5)
            bowl_idx = self.gym.get_actor_rigid_body_index(env_ptr, self.bowl, 0, gymapi.DOMAIN_SIM)
    ...
    def reset(self):
        ...
        self.stir_state_reset()
    
    def scoop_state_reset(self):
        self.is_scooping = False
        self.scoop_up = False
        self.scoop_critical = False
        self.finish = False
        self.scoop_delta = 0.3
        
    def stir_state_reset(self):
        self.stir_critical = False
    ...
    def scoop(self):
        bowl_pos = self.rb_state_tensor[self.bowl_indices, :3]
        ...
        if not self.is_scooping and not self.scoop_critical:
            critical_pos = bowl_pos + torch.Tensor([[-0.08, 0, 0.3]])
            critical_rot = torch.Tensor([[1.0, 0.25, 0.0, 0.0]]).to(self.device)
            ...
            to_critical = critical_pos - hand_pos
            critical_dist = torch.norm(to_critical, dim=-1).unsqueeze(-1)
            to_axis = critical_rot[:, :3] - hand_rot[:, :3]
            axis_dist = torch.norm(to_axis, dim=1).unsqueeze(-1)
            w_dist = critical_rot[:, -1] - hand_rot[:, -1]
            w_dist = abs(w_dist)
            self.scoop_critical = critical_dist < goal_offset and axis_dist < axis_offset and w_dist < w_offset
            pos_err = torch.where(critical_dist > goal_offset, critical_pos - hand_pos, torch.Tensor([0, 0, 0]))
            orn_err = torch.where(axis_dist > axis_offset or w_dist > w_offset, self.orientation_error(critical_rot, hand_rot), torch.Tensor([0, 0, 0]))
        
        
        else:
            if not self.is_scooping:
                print("reaching critical pose")
                # goal_pos, _ = torch.sort(self.rb_state_tensor[self.ball_handle_list, :3], dim=0)
                # goal_pos, _ = goal_pos.median(dim=0)
                
                goal_pos = bowl_pos + torch.Tensor([[-0.08, -0.08, 0.23]])
            ...
            self.scoop_up = goal_dist < goal_offset and axis_dist < axis_offset and w_dist < w_offset
            self.finish = self.is_scooping and (goal_dist < goal_offset and axis_dist < axis_offset and w_dist < w_offset)
    ...
    def stir(self):
        ...
        if not self.is_stirring and not self.stir_critical:
            critical_pos = bowl_pos + torch.Tensor([[-0.05, 0, 0.3]])
            critical_rot = torch.Tensor([[1.0, 0.25, 0., 0.]]).to(self.device)
            ...
            w_dist = critical_rot[:, -1] - hand_rot[:, -1]
            w_dist = abs(w_dist)
            self.stir_critical = critical_dist < goal_offset and axis_dist < axis_offset and w_dist < w_offset
            pos_err = torch.where(critical_dist > goal_offset, critical_pos - hand_pos, torch.Tensor([0, 0, 0]))
            orn_err = torch.where(axis_dist > axis_offset or w_dist > w_offset, self.orientation_error(critical_rot, hand_rot), torch.Tensor([0, 0, 0]))
        else:
    ...
    def data_collection(self):
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            delta = 0.01
            print_pos = True
            if not (action == "scoop" or action == "" or action == "save"):
                self.scoop_state_reset()
            if not (action == "stir" or action == "" or action == "save"):
                self.stir_state_reset()


            # 
            for evt in self.gym.query_viewer_action_events(self.viewer):
                action = evt.action if (evt.value) > 0 else action
